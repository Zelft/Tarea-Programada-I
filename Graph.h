...
#undef minor
	...
	struct Celda : Shape {
		...
		Celda(Point xy, int ww, int hh) :w{ ww }, h{ hh }, pos{ xy }
		{
			if (h <= 0 || w <= 0) error("Bad rectangle: non-positive side");
			add(xy);
		}
		Celda(Point xy, int ww, int hh, string n) :w{ ww }, h{ hh }, pos{ xy }, numero{n}
		{
			if (h <= 0 || w <= 0) error("Bad rectangle: non-positive side");
			add(xy);
		}
		Celda(Point x, Point y) :w{ y.x - x.x }, h{ y.y - x.y }
		{
	...
	struct Heap : Shape {
		...
		Heap(Point x, Point y) :w{ y.x - x.x }, h{ y.y - x.y }
		{
			if (h <= 0 || w <= 0) error("Bad rectangle: first point is not top left");
			add(x);
		}
		...
		int height() const { return h; }
		int width() const { return w; }
		//vector<Celda> getCeldas() { return celdas; }
	private:
		//vector<Text *> enumeraciones;
		int h=500;			// height
		int w=1200;			// width
		//vector<Celda> celdas;
	//	Color fcolor;	// fill color; 0 means "no fill"
	...
	struct NodoGUI :Shape{
		//NodoGUI() :pos{ Point(0,0) }, valor{ 0 }{}
		NodoGUI(Point xy, int t) :pos{ xy }, valor{ t }, activo{false}
		{}
		//NodoGUI(Point x, Point y) :w{ y.x - x.x }, h{ y.y - x.y }
		//{
		//}
		void draw_lines() const;
		...
		Point getPos() const { return pos; }
		void setValor(int t) { valor = t;
		//this->draw();
		this->draw_lines();
		}
		void setEstado(bool a) { activo = a; };
	...
	struct Arco :Shape {
		Arco(){}
		Arco(int a, int b, Point x, Point y, int t) :origen{ x }, destino{ y }, tipo{ t }, idxOrigen{ a }, idxDestino{ b }{};
		void draw_lines() const;
		Point getOrigen() { return origen; }
		int idxOrigen;
		int idxDestino;
	private:
		Point origen;
	...
	struct Text : Shape {
		...
		Text(){}
		Text(Point x, const string& s) : lab{ s }, col{Color::black} { add(x); }
		Text(Point x, const string& s, int c) : lab{ s }, col{ c} { add(x); }
		Text(Point x, const string& s, int f, int z) : lab{ s }, fnt{ f }, fnt_sz{ z }, col{Color::black}{ add(x); }
		Text(Point x, const string& s, int f, int z, int c) : lab{ s }, fnt{ f }, fnt_sz{ z }, col{c}{ add(x); }
		...
		void eliminaLetra(char s) { string nuevoLab = "";
		...
		void agregarLetra(string s) {
			if (lab.size() != 0) { lab += ","; lab += s; }
			else {lab += s;}
		}
		void limpiar() { lab = ""; }
		string getLabel() { return lab; }
	private:
		string lab;	// label
		Font fnt{ fl_font() };
		int col;
